...
```{r}
 ...
 for (i in seq_along(methods)) {
     ...
     test <- dfmat %>%
         dfm_group(groups = username) %>%
         textstat_dist(method = sim_method) 
     
     testm <- as.matrix(test) 
        # 与 May 的距离 
   testdf <- as.data.frame(testm[23, c(1:22, 24)])
    
   colnames(testdf) <- "dist_may"
  testdf <- tibble::rownames_to_column(testdf, "username")
   testdf$method <- sim_method
...
ggplot(testdf_all) +
     ...
     coord_flip() +
    xlab("MP username") +
    ylab("Distance score (Lower is more similar)") + 
     theme_minimal()
```
...
```{r}
readability_scores <- textstat_readability(speeches$text, measure = c("Flesch.Kincaid", "ARI", "Coleman.Liau", "SMOG"))

# 将结果赋值回 speeches
speeches$Flesch.Kincaid <- readability_scores$Flesch.Kincaid
speeches$ARI <- readability_scores$ARI
speeches$Coleman.Liau <- readability_scores$Coleman.Liau
speeches$SMOG <- readability_scores$SMOG

# 汇总所有指标
sum_corpus <- speeches %>%
  group_by(speaker) %>%
  summarise(
    mean_FK = mean(Flesch.Kincaid, na.rm=TRUE),
    mean_ARI = mean(ARI, na.rm=TRUE),
    mean_CL = mean(Coleman.Liau, na.rm=TRUE),
    mean_SMOG = mean(SMOG, na.rm=TRUE)
  )
# 看其中一个，比如 ARI：
ggplot(speeches, aes(x=speaker, y=ARI)) +
  geom_bar(stat="summary", fun="mean") + # ggplot 统计均值
  coord_flip() +
  xlab("Speaker") +
  ylab("Mean ARI Complexity") + 
  theme_minimal()
```
